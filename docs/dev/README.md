...
- [Overview Developer Docs](#overview-developer-docs)
  - [Part 1](#part-1)
    - [User docs](#user-docs)
    ...
    - [Templates](#templates)
      - [Blocks](#blocks)
      - [Static folder (see themes first)](#static-folder-see-themes-first)
      - [Critical css (see themes first)](#critical-css-see-themes-first)
    - [Websites](#websites)
  ...
  - [part 2](#part-2)
    ...
    - [publication pipeline](#publication-pipeline)
      - [diag-literature](#diag-literature)
      - [parse publications](#parse-publications)
      - [bibliography](#bibliography)
    - [imgoptim](#imgoptim)
      - [optimize images](#optimize-images)
  - [part 3](#part-3)
    - [netlify](#netlify)
    - [hosting (yourhosting)](#hosting-yourhosting)
    - [dns](#dns)
    - [assets](#assets)
    - [deploy](#deploy)
    - [github actions / website deployment pipeline](#github-actions--website-deployment-pipeline)
      - [deploy-master.yml](#deploy-masteryml)
      - [deploy-develop.yml](#deploy-developyml)
      - [images.yml](#imagesyml)
    - [webteam deploy bot](#webteam-deploy-bot)
      - [webteam deploy bot, used for deploying the wesbites](#webteam-deploy-bot-used-for-deploying-the-wesbites)
      - [two secrets](#two-secrets)
    - [development branch / deploy](#development-branch--deploy)
    - [security](#security)
    - [google analytics](#google-analytics)
    - [colofon](#colofon)
    - [techdocs and documentation website](#techdocs-and-documentation-website)


## Part 1

### User docs
The user documentation can be found in docs/. This documentation explains users how to add/change content. In addition it includes some advanced topics about css, redirects and website configuration. A new webteam member should become familiar with all the user documentions. 
...
- ### Locally 
    To get website local running, the following steps need to be taken:  
        - Clone repository  
        - Install requirements: Linux environment (on Windows via WSL) and Python 3 (we use Ubuntu with conda/miniconda istalled and a conda environment)  
        - Follow the steps [here](https://github.com/DIAGNijmegen/website-content#building-the-website-locally)

- #### Docker
    A dockerfile is provided to build a container (in progress)
...
#### Blocks
Blocks are pieces of code that can be used in multiple pages. For example there is a code block that can be used to show the current page's location (breadcrumbs). For example, it shows: Home / Members / -Member name- when you are visiting the page -Member name-. 
...
When a site is rendering, it may take quite some time to load all defined css assest, while maybe only a part of all these assest are being used. Therefore, we extract the critical css assets (automatically?), and only these css assets are loaded in the header of the page. Other non-critical css assets can then be loaded in the footer. 

More information here: [here](https://github.com/DIAGNijmegen/website-content/blob/master/docs/dev/critical-above-the-fold-css.md). 

### Websites
general info here. Separate folders containing the main structure 
...
Custom pages can be made by (1) overwriting a default page, or (2) by creating an independent page. More info [here](https://github.com/DIAGNijmegen/website-content/blob/master/docs/create-custom-page.md).
...
- #### copy_content and parse_content -> see plugins/content-aggregator
    The copy_content.sh script copies content from the main folder to a website folder (this is also one of the first scripts you need to run in order to build a website locally). Within the copy_content.sh script, the default website pages are copied (such as the home page, 404 and the colofon), the content bib files are copied, and the parse_content.sh script is called which copies only the relevant site specific content to the website folder. Later down the website building pipeline some of the files of the websites' content folders are generated automatically by --BY WHAT--?.  

- #### parse_publication -> see publication pipeline
    This script is called when building the website to download and parse the publication files.
    More info [here](https://github.com/DIAGNijmegen/website-content/blob/252664ac67e6e8c4bd11e1d2325bd68a582d045c/docs/publications-pipeline-developers.md).
...
- #### bootsrapify
    The bootstrap plugin contains many predefined classes for html, which makes it easy to set up the desired layout of our websites.
...
- #### inline extend
    This plugin allows us to make clickable links on our website with relative ease. --double check how we use this
